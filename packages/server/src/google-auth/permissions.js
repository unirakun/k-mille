...
const checkPermissions = async (ctx) => {
  ...
  console.log(`[google-auth] getting google profile of ${profile.name} (${profile.id})`) // eslint-disable-line no-console
...
module.exports = ({ oAuth2 }) => async (ctx, next) => {
  ...
  if (!profile) {
    console.error('[google-auth] no cookie') // eslint-disable-line no-console
  ...
  if (!tokens) {
    console.error('[google-auth] no tokens in cookies') // eslint-disable-line no-console
    ctx.response.status = 401
    return
  }
